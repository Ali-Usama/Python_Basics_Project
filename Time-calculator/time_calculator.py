def add_time(start, duration, opt_day=None):

	#Extracting data...
    start_h = int(start.split(":")[0])
    if "PM" in start: #Converting time to 24 hour format as that would be easier to deal with
        start_h += 12
    start_min = int(start.split(":")[1].split()[0])         # as start time format is 3:00 PM
    ...
    if fut_h >= 12: #Calculating PM or AM and converting time back to AM PM format       
        fut_h -= 12
        if fut_h == 0:	#so that 1am isnt shown as 0
            fut_h = 12
        PM = True
    if fut_h == 0:	#so that 1am isnt shown as 0
            fut_h = 12
    ...
    if opt_day == None:  #If day isnt requested
        if days_later > 1:
            if PM:
        ...
        day = days[(days.index(opt_day.capitalize()) + 1 + days_later) % 7 - 1] #Calculating day
        if days_later > 1:
            if PM:
